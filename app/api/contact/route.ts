...
function buildBookingTeamHtml(payload: EmailPayload) {
    return buildBookingEmailHtml(payload, 'team')
}

function buildBookingTeamText(payload: EmailPayload) {
    return buildBookingEmailText(payload, 'team')
}

function buildBookingEmailHtml(
    payload: Omit<EmailPayload, 'source'> & { meetingType?: string; bookingLinks?: BookingLinks | null },
    variant: 'team' | 'requester',
) {
...
function buildBookingEmailText(
    payload: Omit<EmailPayload, 'source'> & { meetingType?: string; bookingLinks?: BookingLinks | null },
    variant: 'team' | 'requester',
) {
    const platformLabel = resolveBookingPlatformLabel(payload.bookingLinks)
...
function buildBookingDetailRow(label: string, value: string) {
...
function buildOptionalNotesHtml({ heading, body }: { heading: string; body: string }) {
    return `
...
function buildTeamRequesterDetailsHtml(
    payload: Omit<EmailPayload, 'source'> & { meetingType?: string; bookingLinks?: BookingLinks | null },
) {
    const detailItems = [
...
function resolveBookingPlatformLabel(bookingLinks?: BookingLinks | null) {
...
function formatTimeOnlyInTimeZone(date: Date, timeZone: string) {
    try {
